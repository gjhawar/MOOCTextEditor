...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean lastVowelFlag = false;

		for(int i=0;i<charArray.length;i++){
			//If last element is 'e' and there have been syllables before then don`t count it.
			if(i==charArray.length-1 && charArray[i]=='e' && syllableCount>0){
				break;
			}
			if(charArray[i]=='a' || charArray[i]=='e'||charArray[i]=='i'|| charArray[i]=='o'|| charArray[i]=='u'|| charArray[i]=='y'){
				// Takes care of checking repetitive vowels and count them only once
				if(lastVowelFlag == false){
					lastVowelFlag = true;
					syllableCount++;
	...
	public static boolean testCase(Document doc, int syllables, int words, int sentences)
	{
		...
		System.out.println("Flesch score is : "+ fleschScore + "\n");
	...
	public double getFleschScore()
	{
		...
		if(getNumWords()>0 && getNumSentences()>0 && getNumSyllables()>0)
		{
