...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2)
	{
		...
		// insert first node
		WPTreeNode startNode = new WPTreeNode(word1, null);
		...
		while(!queue.isEmpty() && !foundPath){
			...
			for(String neighbor : neighbors){
				if(!visited.contains(neighbor)){
					WPTreeNode childNode = curr.addChild(neighbor);
					queue.add(childNode);
			...
			System.out.printf("Current node is : %s", curr);
			printQueue(queue);
		}
	    return retList;
	}
...
class WPTreeNode {
    ...
    public WPTreeNode addChild(String s){
    	//Add this as the parent of the child
    ...
    /** Allows you to build a path from the  to
     *  the calling object to root node
